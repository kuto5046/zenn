...
例えばResearchエージェントの場合、web検索を行い外部から得られる情報が観測に相当します。Agentic RLにおいては状態は=コンテキストと考えても良いかもしれません。
...
たとえばZeroSearch[259]は、リアルなウェブ検索をLLM自体から精錬された疑似検索エンジンで置き換え、実際のクエリを発行することなくカリキュラムRLを組み合わせてリアルエンジンの性能へ段階的に近づけます。SSRL[260]はこのアイデアをさらに進化させ、トレーニング中に明示的な検索エンジンを使わずに、完全にオフラインで「セルフサーチ」を行い、オンライン推論時にはリアルなAPIで性能をさらに引き上げることができる。これらの研究はまだ始まったばかりではありますが、オフラインセルフサーチはAPIの限界を超えて安定性とスケーラビリティを高め、より自立した研究エージェントへの道を示しています。

- ZeroSearch
- SSRL

このようにRAGとRLの組み合わせによる検索性能の向上には進展があるものの、ほとんどのオープンソースモデルはエージェントが見つけにくい情報をどのように特定できるかを測る難易度の高いベンチマークであるOpenAIのBrowseCompで依然として失敗しています。一方クローズドモデルであるChatGPT Deep ResearchはBrowseCompでpass@1=51.5%を達成しており性能の高さが伺えます。ChatGPTをはじめとするクローズドのDeep ResearchモデルはRLを用いて検索ツールやメモリを最適化することで、より効果的な情報探索を実現しています。 -->
...
### 数学エージェント
数学的推論は、その記号的な抽象性、論理的な一貫性、そして長期にわたる演繹が求められるという性質から、LLMエージェントの推論能力を評価する上で極めて重要な基準と考えられています。
エージェントのコア能力で紹介した研究でも数学的なタスクに対する性能を見ているものが数多くありました。

ここではそれとは違う紹介します。
- 1shot RLVR

rStar2-Agentは難しい数学的タスクに対して、推論データによるSFTなしの純粋なAgentic RLアプローチによって14Bパラメータで671BのDeep Seek R1 Zeroを上回る性能および学習効率を達成しています。
これは、単に「長く考える(thinking longer)」だけでなく、「賢く考える(thinking smarter)」ことの重要性を示しています。rStar2-Agentは、pythonGRPO-RocというGRPOを改良した新しいRLアルゴリズムを導入しています。GRPO-Rocでは、


### GUIエージェント
GUIエージェントはwebブラウザ操作やアプリケーション操作といったタスクを自律的に行うエージェントです。
研究初期はVLMを用いてスクリーンショットとプロンプトを入力として単一ステップのGUI操作を行う方法が提案されました。その後過去のGUI捜査実績をもとに画面(状態)とGUI操作(行動)との軌跡データを元にSFTでGUI操作を模倣学習する方法が試みられましたが、人間によるGUI操作記録のデータセットが乏しいという制約がありました。このような背景からGUI操作をRLで最適化する研究が進展しています。
RLを用いたGUIエージェントの研究は大きく2つの方向性があります。1つは事前に蓄積した静的なGUI操作データセットを用いてオフラインでRLを行う方法、もう1つはインタラクティブにGUI環境とやり取りしながらオンラインでRLを行う方法です。ここでは後者の研究について紹介します。
- UI-TARS
...
VLN-R1 [413]は、予測された経路と正解の経路を一致させることで軌跡レベルの報酬を定義し、DeepSeek-R1に倣ってGRPOを適用し、予測的な計画能力を向上させます。

#### マニピュレーションエージェント
マニピュレーションエージェントは主にロボットアームが関わるタスクで利用されます。RLはVLAモデルの指示追従能力と軌跡予測能力を強化するために、特にタスクや環境を越えた汎化性能を向上させる目的で用いられます。

- VLA-RL [412]は、事前学習済みのVLMを評価者として採用し、そのフィードバックを用いてVLAポリシーの改善のための軌跡レベルの報酬を割り当てます。これらの手法は、マニピュレーション性能を効果的に向上させ、良好なスケーリング特性を示すオンラインRLフレームワークを確立します。
