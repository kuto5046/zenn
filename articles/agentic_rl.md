...
ほぼ同時期に発表されたARTISTも似たようなアプローチをとっています。ARTISTは数学的タスクだけでなく、多段階のFunction Callingがが必要となるタスクに対して推論, ツール利用を繰り返し行いながらタスクに対して最終的な回答を生成するというプロセスをどのツールをいつ使うかを学習します

![alt text](image-8.png)
#### 長期的・マルチステップのツール使用
上記で示した強化学習を利用したツール統合型推論は、研究領域だけでなくChatGPT Deep ResearchやOpenAI o3のような商用システムにもすでに採用されています。今後の研究の方向性として、長期的なステップでのツール連携や、複数のツールの組み合わせによる複雑なタスク解決が挙げられています。
- GiGPO

これが可能になればエージェントは複雑なツールチェーンを自律的に学び、未知のタスクにも柔軟に対処できるようになると期待されています。
